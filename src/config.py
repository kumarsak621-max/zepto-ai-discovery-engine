...
# Also support Railway / platform env without .env file
load_dotenv()
...
# Server bind (used by app.py launcher / Procfile)
...
def validate_runtime_config() -> list[str]:
    ...
    if not has_gemini():
        warnings.append(
            ...
            "fallbacks until you add the key in Railway Variables / .env."
        )
    if not has_reddit():
        warnings.append(
            "Reddit credentials are optional. Set REDDIT_CLIENT_ID and REDDIT_SECRET "
            "to enable Reddit collection."
        )
    try:
        DATABASE_PATH.parent.mkdir(parents=True, exist_ok=True)
    except OSError as exc:
        warnings.append(f"Cannot create database directory {DATABASE_PATH.parent}: {exc}")
    return warnings
