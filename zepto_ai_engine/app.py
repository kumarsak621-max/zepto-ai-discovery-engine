...
if str(ROOT) not in sys.path:
    ...
    print(sys.path)
from src.config import has_gemini, has_reddit
...
c4.metric("Last update", (stats.get("last_update") or "—")[:19] if stats.get("last_update") else "—")
...
if stats.get("by_source"):
    ...
    st.warning(
        ...
        "or configure API keys in `.env` and re-run."
    )
